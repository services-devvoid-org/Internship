...
const AboutMe = () => {
  return (
    <div id="aboutMe"
    className='bg-slate-300 w-screen h-screen flex justify-between'>
      ...
      <div className='w-full flex flex-col p-6 mt-28 mr-8'>
        ...
        {skills.map((skill,index)=>(
          <li key={index} className='px-2 font-bold text-rose-600 rounded-lg'>{skill}</li>
        ))}
          </ul>
          <ul className='flex flex-col mt-6'><span className='font-semibold'>Education: </span>
        {education.map((edu,index)=>(
          <li key={index} className='px-2 flex ml-4 font-bold text-rose-600 rounded-lg'>{index+1}. {edu.College}- {edu.Degree} ({edu.CGPA})
          </li>
        ))}
          </ul>
          <ul className='flex flex-col mt-6'><span className='font-semibold'>Experience: </span>
        {experience.map((exp,index)=>(
          <li key={index} className='px-2 flex ml-4 font-bold text-rose-600 rounded-lg'>{index+1}. {exp.title}: {exp.Duration}<br/>{exp.Description}
          </li>
        ))}
          </ul>
      </div>
